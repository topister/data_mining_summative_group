...
def main():
    ...
    st.title('Financial Inclusion Predictor: Empowering Access to Banking Services for All')
    ...
    if st.button("Predict"):
        ...
        encoded_user_data = encoder.transform(user_data[['country', 'location_type', 'cellphone_access',
                                                         'gender_of_respondent', 'relationship_with_head',
                                                         'marital_status', 'education_level', 'job_type']])

        # Convert the encoded data to a DataFrame
        encoded_df = pd.DataFrame(encoded_user_data.toarray(), columns=encoder.get_feature_names_out(
